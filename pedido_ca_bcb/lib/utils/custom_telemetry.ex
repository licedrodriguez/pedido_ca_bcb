defmodule PedidoCaBcb.Utils.CustomTelemetry do
  alias PedidoCaBcb.Utils.DataTypeUtils
  import Telemetry.Metrics

  @service_name Application.compile_env!(:pedido_ca_bcb, :custom_metrics_prefix_name)
  ...
  def custom_telemetry_events() do
    :telemetry.attach("pedido_ca_bcb-plug-stop", [:pedido_ca_bcb, :plug, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("pedido_ca_bcb-redis-stop", [:redix, :pipeline, :stop], &handle_custom_event/4, nil)
    :telemetry.attach("pedido_ca_bcb-vm-memory", [:vm, :memory], &handle_custom_event/4, nil)
  ...
  def handle_custom_event([:pedido_ca_bcb, :plug, :stop], measures, metadata, _) do
    :telemetry.execute(
